<template>
  <q-page class="q-pa-md menu-page">
    ...
    <q-dialog v-model="createDialog">
      <ShortLinkForm :item="createItem" @create="onCreate" />
    </q-dialog>

    <!-- Dialog Meus Links -->
    <q-dialog v-model="listDialog">
      <ShortLinkList :items="items" />
    </q-dialog>
...
import type { NewShortLink } from 'src/schemas/ShortLink.Schemas';
    ...
    async onCreate(newShortLink: NewShortLink) {
      try {
        return this.$load.execute('links.createLink', async () => {
          const created = await this.linksComposable.createLink(newShortLink);
          this.createDialog = false;
          this.createItem = { originalUrl: '' } as NewShortLink;
        });
      } catch (err) {
